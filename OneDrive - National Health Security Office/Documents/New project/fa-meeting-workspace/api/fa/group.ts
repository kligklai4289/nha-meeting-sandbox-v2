...
const schema = z.object({
  ...
  presenter: z.string().max(500),
  status: z.enum(['draft', 'review_ready']),
}).strict()

export function createFaGroupHandler(gateway: FaMutationGateway, signingSecret: string, now?: () => Date) {
  return createFaMutationHandler({
    gateway, signingSecret, now, method: 'PATCH', kind: 'group_update', schema,
    payload: ({ presenter, status }) => ({ presenter, status }),
  })
}
...
const handler = {
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      return createFaGroupHandler(new SupabaseFaMutationGateway(createSupabaseServerClient(env)), env.FA_SESSION_SIGNING_SECRET).fetch(request)
    } catch {
